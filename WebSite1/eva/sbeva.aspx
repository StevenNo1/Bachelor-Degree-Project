<%@ Page Title="" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="sbeva.aspx.cs" Inherits="_Default" %>

<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" Runat="Server">
        <br>
    <div>
        <br>
        <asp:Label ID="Label1" runat="server" Text="我的賣場之客戶評價" style="font-size: x-large"></asp:Label>
        <br>
        <br>
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:Anotherconnection %>" DeleteCommand="DELETE FROM [Evaluation] WHERE [eNo] = @eNo" InsertCommand="INSERT INTO [Evaluation] ([pName], [toMid], [level], [depiction]) VALUES (@pName, @toMid, @level, @depiction)" SelectCommand="SELECT [eNo], [pName], [toMid], [level], [depiction] FROM [Evaluation] WHERE (([submitMid] = @submitMid) AND ([level] IS NULL) AND ([type] = @type)) ORDER BY [eNo]" UpdateCommand="UPDATE [Evaluation] SET [pName] = @pName, [toMid] = @toMid, [level] = @level, [depiction] = @depiction WHERE [eNo] = @eNo">
            <DeleteParameters>
